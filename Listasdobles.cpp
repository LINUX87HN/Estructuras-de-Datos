	...
	if (primero!=NULL)
{
	...
	{
		if(Q==primero)// si es le prime nodo
		{
			primero=Q->siguiente;
			if(Q->siguiente!=NULL)//ni en el primer nodo ni en el ultimo nodo.
			Q->siguiente->anterior=NULL;
		}
		else if(Q->siguiente!=NULL) //no es ni el primero ni el ultimo
		{
		
			T->siguiente=Q->siguiente;// Q->anterior->siguiente=Q->siguiente
			Q->siguiente->anterior=T;// Q->siguiente->anterior=Q->anterior
		}
 ...
 void lista_doble::imp_ant_sig( int vref)//imprime el siguiente y anterior nodo a un nodo con valor vref
{
	if (primero!=NULL)
{
		pnodo_doble Q;
...
listad1.imp_ant_sig(2);/*
listad1.imprime();
listad1.borrar_vref(2);
listad1.imprime();*/
   return 0;
}  
